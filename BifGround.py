...
if fault.isdigit():
    ...
    if fault < 1 or fault > len(secs_diagonal['Z0_diagonal']):
        ...
        # Secuence Current
        I1 = (Vf)/(Z1_fault + ((Z2_fault*(Z0_fault + 3*Zf))/(Z2_fault + Z0_fault + 3*Zf)))
        I0 = -I1*(Z2_fault/(Z2_fault + Z0_fault + 3*Zf))
        I2 = -I1*((Z0_fault + 3*Zf)/(Z2_fault + Z0_fault + 3*Zf))
    ...
    for num_bars, values in fault.items():
        ...
        # Bif-Ground Fault
        I1 = (Vf)/(values['Z1_fault'] + ((values['Z2_fault']*(values['Z0_fault'] + 3*Zf))/(values['Z2_fault'] + values['Z0_fault'] + 3*Zf)))
        I0 = -I1*(values['Z2_fault']/(values['Z2_fault'] + values['Z0_fault'] + 3*Zf))
        I2 = -I1*((values['Z0_fault'] + 3*Zf)/(values['Z2_fault'] + values['Z0_fault'] + 3*Zf))
